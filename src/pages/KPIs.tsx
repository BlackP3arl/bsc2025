...
export const KPIs: React.FC = () => {
  ...
  const kpisByFrequency = frequencies.map(freq => ({
    ...freq,
    kpis: kpiDefinitions?.filter(kpi => kpi.frequency === freq.value) || [],
  }));

  const totalKPIs = kpiDefinitions?.length || 0;
  const activeKPIs = kpiDefinitions?.filter(kpi => kpi.status === 'Active').length || 0;
  const draftKPIs = kpiDefinitions?.filter(kpi => kpi.status === 'Draft').length || 0;
